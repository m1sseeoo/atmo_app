...
Missing readings stay absent (`undefined`). Pressure is not synthesized when its sensor is unavailable.
...
The Comfort Index uses only available measurements. A missing pressure sensor contributes neither a good nor a bad value. Rule-based recommendations use the saved user profile and form the deterministic safety/fallback layer for the assistant.
