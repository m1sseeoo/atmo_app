...
function evaluateUv(reading: DeviceReading, profile: UserProfile | null | undefined, impacts: ComfortFactorImpact[]): void {
  const value = reading.uvIndex;
  if (value === undefined || value <= 2) return;
  let points = value <= 5 ? -4 : value <= 7 ? -10 : -18;
  if (hasFactor(profile, 'Солнце / УФ')) points -= 5;
  pushPenalty(impacts, { key: 'uv', label: 'Повышенный УФ', points, explanation: 'Солнечная нагрузка повышена.' });
}
...
function countAvailable(reading: DeviceReading): number {
  return [reading.temperature, reading.humidity, reading.pressure, analyzeAirEnvironment(reading).status === 'unknown' ? undefined : 1, reading.rain ?? reading.rainProbability, reading.uvIndex, reading.windSpeed].filter((value) => value !== undefined).length;
}
...
export function calculateComfortIndex(reading: DeviceReading, profile?: UserProfile | null): ComfortIndexResult {
  ...
  evaluateUv(reading, profile, impacts);
