...
export function detectAssistantIntent(message: string): AssistantIntent {
  ...
  if (matchesAny(text, ['уф', 'uv', 'солнце', 'солнечно', 'загар'])) {
    return 'uv';
  }
...
function replyUv(reading: DeviceReading): string {
  if (reading.uvIndex === undefined) return 'УФ-датчик не подключён, поэтому данных об УФ-индексе нет.';
  if (reading.uvIndex >= 8) {
    return `УФ-индекс высокий: ${reading.uvIndex}. Солнечная нагрузка заметная — лучше использовать защиту от солнца и выбирать тень.`;
  }
  if (reading.uvIndex >= 5) {
    return `УФ-индекс: ${reading.uvIndex}. Солнечная нагрузка повышена — стоит использовать головной убор и защиту от солнца.`;
  }
  return `УФ-индекс сейчас ${reading.uvIndex}. Солнечная нагрузка умеренная. Ориентируйтесь на своё самочувствие при длительном пребывании на солнце.`;
}
...
export function generateLocalAssistantReply(params: AssistantReplyParams): string {
  ...
  switch (intent) {
    ...
    case 'air_quality':
      ...
      break;
    case 'uv':
      reply = replyUv(reading as DeviceReading);
      break;
    case 'rain':
