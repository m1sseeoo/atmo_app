...
export function buildAnalyticsMetricsFromReading(reading: DeviceReading): AnalyticsMetric[] {
  ...
  if (reading.uvIndex !== undefined) metrics.push({ title: 'УФ-индекс', value: String(reading.uvIndex), level: reading.uvIndex >= 6 ? 'Высокий' : 'Умеренный', levelColor: '#1688E8', explanation: 'Последнее доступное измерение.', color: '#1688E8', points: currentOnly(reading.uvIndex) });
  return metrics;
}
